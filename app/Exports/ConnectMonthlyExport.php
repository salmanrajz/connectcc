...
class ConnectMonthlyExport implements FromCollection
{
    /**
    ...
    */
    public function collection()
    {
        ...
        if ($time == '1') {
            ...
            // \DB::statement(\DB::raw('set @count=0'));
            // return $query->select(\DB::raw('*, (@count:=@count+1) AS serial'));
            return $data = activation_form::select(\DB::raw('(@count:=@count+1+1) AS serial'), 'activation_forms.activation_selected_no', 'activation_forms.activation_sr_no', 'plans.plan_name', 'activation_forms.sim_type', 'activation_forms.activation_date')
            ->LeftJoin(
                'plans',
                'plans.id',
                'activation_forms.select_plan'
